...
} from "./commands/commandToUpdateComputerArray";
import {
  WhiteboardAddUserCommand,
  WhiteboardRemoveUserCommand,
} from "./commands/commandToUpdateWhiteboard";
import ChatMessageUpdateCommand from "./commands/commandToUpdateChatMessage";
...
export class CampusMetaverse extends Room<RoomState> {
  ...
  async onCreate(options: RoomData) {
    ...
    this.onMessage(
      ...
      (client, message: { x: number; y: number; animation: string }) => {
    ...
    this.onMessage(
      Message.DISCONNECT_PRESENTATION,
      (client, message: { clientId: string }) => {
        this.clients.forEach((cli) => {
          if (cli.sessionId === message.clientId) {
            cli.send(Message.DISCONNECT_PRESENTATION, client.sessionId);
          }
        });
      }
    );
